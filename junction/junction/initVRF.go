...
func InitVRF() (success bool, addr string) {
	...
	privateKeyStr := junction2.GetVRFPrivateKey()
	if privateKeyStr == "" {
		return false, ""
	}
	...
	publicKey := junction2.GetVRFPubKey()
	if publicKey == "" {
		return false, ""
	}
	...
	serializedRC, err := junction2.SerializeRequestCommitmentV2Plus(rc)
	if err != nil {
		logs.Log.Error(err.Error())
		return false, ""
	}

	proof, vrfOutput, err := junction2.GenerateVRFProof(suite, privateKey, serializedRC, int64(rc.BlockNum))
	if err != nil {
		fmt.Printf("Error generating unique proof: %v\n", err)
		return false, ""
	}

	extraArg := types2.ExtraArg{
	...
	latestVerifiedBatch := junction2.QueryLatestVerifiedBatch()
	...
	for {
		...
		for {
			...
			if errTxRes != nil {
				...
				if strings.Contains(errStr, junction2.VRFInitiatedErrorContains) {
