...
func CreateStation(extraArg types2.StationArg, stationId string, stationInfo types.StationInfo, accountName, accountPath, jsonRPC string, verificationKey groth16.VerifyingKey, addressPrefix string, tracks []string, bootstrapNode []string) bool {
	...
	stationData := types2.MsgInitStation{
		Creator:           newTempAddr,
		Tracks:            tracks,
	...
	successGenesis := junction2.CreateGenesisJson(stationInfo, verificationKey, stationId, tracks, tracksVotingPower, txResp.TxHash, timestamp, extraArg, newTempAddr)
	if !successGenesis {
		return false
	}
